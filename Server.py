...
class Server(object):
    def __init__(self,client_num,task_num,dataset,lr,device,epoch_local,pseudo_samples,global_method,local_method,
                 FE,subset,batch_size,alpha,test_dataset,class_num):
    ...
    def init_clients(self):
        ...
        for i in range(self.client_num):
            self.clients.append(Client(self.dataset_name,self.learning_rate,self.device,self.task_num,i,
                                       ...
                                       self.data[i],self.batch_size,self.alpha
                                       ))
        print("初始化完成")
    ...
    def model_global_eval(self,taskid):
        self.test_dataset.getTestData([0, self.class_num])
        print(self.global_models.keys())
        self.test_loader = DataLoader(self.test_dataset,batch_size=self.batch_size,shuffle=True)
        acc = prediction(self.global_models,self.test_loader,self.device)
        print(f'第{taskid}回合结束，global model的准确率为{acc}')
